...
export default function AdminPostDetailPage(): JSX.Element {
  ...
  const [post, setPost] = useState<AdminPostType | null>(null);
  ...
  const loadPost = async (): Promise<void> => {
    try {
      setLoading(true);
      const fetchedPost: AdminPostType = await fetchPostById(postId);
      setPost(fetchedPost);
    } catch (error) {
  ...
  const handleToggleFeature = async (): Promise<void> => {
    if (!post) return;
  ...
  const handleToggleVisibility = async (): Promise<void> => {
    ...
    try {
      const hidden: boolean = post.status !== 'hidden';
  ...
  const getPostTypeIcon = (type: string): JSX.Element => {
    switch (type) {
      case 'general':
