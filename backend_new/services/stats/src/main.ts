...
async function bootstrap() {
  ...
  const logger = new Logger('StatService')
  logger.log('Stat service is starting')
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription('API documentation for the Stats Service')
    .setVersion('1.0')
    .build();
  ...
  Logger.log(`Stats Service is running on http://localhost:${port}`, 'Bootstrap');
