...
export class AddTipFields1749023872004 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."posts_tip_category_enum" AS ENUM('single_bet', 'combo_bet', 'system_bet', 'live_bet')`,
    );
    await queryRunner.query(
      `ALTER TABLE "posts" ADD "tip_category" "public"."posts_tip_category_enum"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "posts" ADD "is_result_set" boolean NOT NULL DEFAULT false`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."posts_type_enum" AS ENUM('tip', 'general', 'discussion', 'analysis')`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."posts_status_enum" AS ENUM('draft', 'published', 'private', 'archived', 'deleted', 'reported')`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."posts_tip_result_enum" AS ENUM('pending', 'won', 'lost', 'void', 'half_won', 'half_lost')`,
    );
    await queryRunner.query(
      `ALTER TABLE "posts" ADD "tip_result" "public"."posts_tip_result_enum" DEFAULT 'pending'`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."posts_status_enum_old" AS ENUM('draft', 'published', 'archived')`,
    );
    await queryRunner.query(`ALTER TABLE "posts" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
      `ALTER TABLE "posts" ALTER COLUMN "status" TYPE "public"."posts_status_enum_old" USING "status"::"text"::"public"."posts_status_enum_old"`,
    );
    await queryRunner.query(`ALTER TABLE "posts" ALTER COLUMN "status" SET DEFAULT 'published'`);
    await queryRunner.query(`DROP TYPE "public"."posts_status_enum"`);
    await queryRunner.query(
      `ALTER TYPE "public"."posts_status_enum_old" RENAME TO "posts_status_enum"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."posts_type_enum_old" AS ENUM('tip', 'discussion', 'news', 'analysis')`,
    );
    await queryRunner.query(`ALTER TABLE "posts" ALTER COLUMN "type" DROP DEFAULT`);
    await queryRunner.query(
