...
async function main() {
  ...
  try {
    ...
    const createPostResponse = await axios.post(
      `${API_BASE}/posts`,
      {
        content: 'This is a test post created for view tracking tests. It should work with the OptionalJwtAuthGuard.',
        type: 'discussion'
      },
      {
        headers: {
    ...
    testPostId = createPostResponse.data.post_id || createPostResponse.data.id;
    console.log(`✅ Test post created with ID: ${testPostId}`);
    console.log(`   Full response: ${JSON.stringify(createPostResponse.data)}`);
    
    if (!testPostId) {
      console.log('❌ Could not extract post ID from response');
      return;
    }
    ...
    console.log('\n📋 Test Summary:');
    console.log('   ✅ User registration and login works');
