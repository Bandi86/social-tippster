...
async function finalVerificationTest() {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `   ❌ /auth/me endpoint - FAILED (${error.response?.status || error.message})`,
        );
        if (error.config?.url?.includes('/api/api/')) {
          testResults.noDoubleApiPrefix = false;
      ...
      try {
        ...
        try {
          ...
          console.log(
            `   ❌ Authenticated view tracking - FAILED (${error.response?.status || error.message})`,
          );
          if (error.config?.url?.includes('/api/api/')) {
            testResults.noDoubleApiPrefix = false;
        ...
        try {
          ...
          console.log(
            `   ❌ Anonymous view tracking - FAILED (${error.response?.status || error.message})`,
          );
        }
      } catch (error) {
  ...
  console.log('=====================');
  console.log(
    `Frontend Server:              ${testResults.frontendOnline ? '✅ ONLINE' : '❌ OFFLINE'}`,
  );
  console.log(
    `Backend Server:               ${testResults.backendOnline ? '✅ ONLINE' : '❌ OFFLINE'}`,
  );
  console.log(
    `User Registration:            ${testResults.registrationWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `User Login:                   ${testResults.loginWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `Auth/Me Endpoint:             ${testResults.authMeWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `Post Creation:                ${testResults.postCreationWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `Authenticated View Tracking:  ${testResults.postViewTrackingWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `Anonymous View Tracking:      ${testResults.anonymousViewTrackingWorks ? '✅ WORKING' : '❌ FAILED'}`,
  );
  console.log(
    `No Double API Prefix:         ${testResults.noDoubleApiPrefix ? '✅ CLEAN URLS' : '❌ DOUBLE PREFIX DETECTED'}`,
  );
